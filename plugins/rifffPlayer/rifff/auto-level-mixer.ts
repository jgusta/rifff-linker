  ...
  processBlock(buffer) {
    for (let i = 0; i < buffer.length; i++) {
      ...
      if (buffer.numberOfChannels > 1) {
        sample = Math.max(
          Math.abs(buffer.getChannelData(0)[i]),
          Math.abs(buffer.getChannelData(1)[i])
        )
      } else {
      ...
      if (this.smallWindowCounter >= this.smallWindowSize) {
        ...
        if (this.rmsHistoryIndex === this.rmsHistorySize) {
          ...
          const differenceComparedToThreshold =
            chosenValueDecibels - this.thresholdInDecibels
          this.levelDifference = Math.max(0.0, differenceComparedToThreshold)

          this.decrementValue =
            this.levelDifference * -1.0 * this.levelReductionFactor
